...
export default function ProductTable({ rows, unit = '백만원' }) {
  return (
        ...
        <tbody>  {/* 실제 데이터 들어가는 곳 */}
          {rows.map((product) =>  // 제품 반복
            product.rows.map((row, index) => {
              ...
              return (
                <tr
                  key={`${product.product}-${row.label}`}  // 각 행을 구분
                  className={index === 0 ? 'product-group-start' : ''}  // 첫 번째 줄이면 product-group-start 적용
                >

                  {index === 0 && (  // 첫 번째 줄일 때만 제품명 출력
                    <td
                      className="col-product"  // CSS 스타일 적용
                      rowSpan={product.rows.length}  // 셀을 여러 줄에 걸쳐 합침(셀 병합)
                    >
                      {product.product}
                    </td>
                  )}
